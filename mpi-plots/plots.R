...
data <- read.table('data_sizek.tsv', header=T)
data <- subset(data, algorithm %in% c('mpi', 'new'))

data$algorithm <- factor(data$algorithm, levels=c('mpi', 'new'), labels=c('MPI', 'CPU'))

ggsave("plots/sizek_mpi.pdf", units="in", width=3.25, height=2,
ggplot(data, aes(cpus, time*cpus, color=algorithm, shape=algorithm, group=algorithm)) +
  geom_point() +
  scale_x_log10("Cores (log-scale)") +
  scale_y_log10("Total wall time (all CPUs, log-scale)", labels=function(x)sitools::f2si(x, unit='s')) + # , labels=c(expression('10'^1*' s'),expression('10'^2*' s'),expression('10'^3*' s'),expression('10'^4*' s')), breaks=c(10,100,1000,10000)) +
  scale_color_brewer("Software", palette='Dark2') +
  scale_shape("Software") +
...
data <- read.table('data_large.tsv', header=T)
