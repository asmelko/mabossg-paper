...
MaBoSS was initially developed as a single-core application, but very quickly it was extended with a basic parallelism to exploit the multi-core nature of modern CPUs. In this parallel implementation the simulation of trajectories, as well as the statistics aggregation, was distributed on multiple cores (controlled by a simulation parameter) using POSIX threads. In the following sections of the papers, we will refer to this implementation as the CPU version. 

\subsubsection{Statistics output and visualization}

Each of the three kinds of statistics is in its nature a sample from a probabilistic distribution of Boolean states. This sample is represented in code as a hash map in the CPU version, varying in the $(key, value)$ pairs according to the specific statistic. For the final states, the keys of the hash map are the states of the model, and the values are the number of times the state was sampled last in a trajectory. Such output can be visualized as a pie chart (see Figure~\ref{fig:cohen_final}). The fixed states are represented similarly, but only the fixed points are stored in the hash map as keys.
