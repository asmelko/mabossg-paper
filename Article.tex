...
MaBoSS simulates the \emph{asynchronous update strategy}, where at most a single node changes its value in each transition. Therefore, there is a transition from $S$ to $S'$ only if it holds that 
\begin{equation}
    \begin{split}
...
The main computational part of a Boolean logic is a \emph{Boolean formula} $B: \mathcal{S} \rightarrow \{0, 1\}$, which consists of logical operators (such as \emph{and}, \emph{or}, \emph{xor}, \emph{not}) with nodes as operands. For example, 
\begin{equation}
    B_i(S) = (S_2 \land S_3) \lor S_4 
\end{equation}
is a Boolean formula for node $i$, where $S_2$, $S_3$ and $S_4$ are binary values of nodes $2$, $3$ and $4$, respectively. Using the values of $B(S)$ and $S_i$, $\mathcal{B}_i(S)$ is defined by a simple rule:
\begin{equation}
...
\State $i \gets \min_i \sum_{j=1}^{i-1} \rho_j \leq r < \sum_{j=1}^{i} \rho_j$ \Comment select index of the transition node
\State $S'\gets S\ \text{with the $i$-th bit flipped}$
\State $u \gets \text{random}([0, 1])$
\State $\delta t \gets -\frac{\ln u}{\sum_{i=1}^n \rho_i}$ \Comment compute time to the next transition
\State \textbf{return} $(S', t + \delta t)$
\EndProcedure
\end{algorithmic}
\end{algorithm}

To obtain a good overview of the Markovian process, we apply the Gillespie stochastic simulation algorithm~\cite{gillespie1976general} (or kinetic Monte-Carlo~\cite{young1966monte}) on the Boolean state space. Multiple trajectories are generated and aggregated in compound trajectory statistics. Commonly obtained statistics include:
\begin{itemize}
