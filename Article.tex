...
Biological systems are very large and complex, and understanding their behavior remains a very important task to allow the design of new therapies for complex diseases such as cancer. A key approach in this endeavor is to build computational models from existing knowledge, and to analyze them to try to find intervention points and predict the efficacy of new treatment.
Many different frameworks have been used to describe biological systems, from very quantitative systems of differential equations, to more qualitative approaches such as Boolean models. While the former seems more adapted to represent complex behavior such as non-linear dependencies, the latter is increasingly used because it allows the representation of very large systems.
Many Boolean models have been built to describe biological systems, to tackle a variety of problems : from understanding basic properties of cell cycle\citep{faure2006cellcycle,sizek2019boolean}, to more advanced properties of cancer \citep{montagud2022prostate} or ...
Historically, the task of building such models involved reading a very large literature and summarizing it in a list of important components and their interactions. More recently, thanks to advances in databases listing such interactions\citep{licata2020signor,turei2016omnipath}, and to experimental techniques providing informations on a larger number of components [rnaseq, single cell, …], methods have been designed to automatically infer Boolean formulas from the constraints encoded in the knowledge and in the experimental data \cite{chevalier2020synthesis,benevs2023boolean} to build large Boolean models. While this effort suffers many challenges, we believe it is the way to go to study the large scale complexity of biological systems. In order to analyse the dynamical properties of such large Boolean models, we need efficient methods, among them for simulations. In this effort, adapting to the modern HPC computing architectures existing softwares in an important goal. 
MaBoSS \cite{stoll2012continuous, stoll2017maboss} is a stochastic Boolean simulator which performs estimations of state probability trajectories, based on Markov chains. 
In this paper, we discuss about the adaptation of MaBoSS simulation algorithm for two different hardware architectures : large CPU clusters, with a MPI parallel implementation, and GPU accelerators.
